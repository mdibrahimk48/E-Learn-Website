...
public function editNotice($data,$id)
{
	...
	if ($title==""||$description==""||$started_at=="") {
    ...
    if ($updateQuiz) {
			$_SESSION['success']="<span class='success'>Notice data updated successful.</span>";
			return $this->rediret();
		}else{
			$_SESSION['error']="<span class='error'>Notice data  not updated .</span>";
			return $this->rediret();
		}
}
public function addNotice($data)
{
   ...
   $query = "INSERT INTO notice(admin_id,title,description,started_at)
    VALUES('$adminid','$title','$description','$started_at')";

    $quizInsert=$this->db->insert($query);
    if ($quizInsert) {
			$_SESSION['success']="<span class='success'>Notice data insert successful.</span>";
			return $this->rediret();
		}else{
			$_SESSION['error']="<span class='error'>Notice data  not insert .</span>";
			return $this->rediret();
		}
}
...
if ($deldata) {
	$_SESSION['success']="<span class='success'>Notice deleted successful</span>";
	return $this->rediret();
}else{
 		$_SESSION['error']="<span class='error'>Notice  not deleted!</span>";
	return $this->rediret();
 	}
}
public function getNotices()
{
	$query="select * from notice order by started_at desc limit  8";
